...
const CONFIG = {
  ...
  JAIL_DURATION_MS: 5 * 60 * 1000, // 5 minutes
};
...
const commands = {
  ...
  '!aide': async (message) => {
    ...
    const e = embed('#5865F2')
      ...
      .addFields(
        ...
        { name: '🎮 Fun / Troll',  value: '`!iqtest [@user]` • `!cope-du-jour` • `!fluide @user` *(admin)*', inline: false },
  ...
  '!rr-attach': async (message, args) => {
    ...
    try {
      ...
      return message.reply(`❌ Message introuvable avec l'ID \`${messageId}\` dans ${targetChannel}.`);
    }
    ...
    if (reactionRolesData[messageId]) {
      return message.reply(
        ...
        `Utilise \`!rr-add ${messageId} | <emoji> | @role\` pour ajouter des rôles.`
      );
    }
  ...
  // ============================================================
  //  🔒  JAIL — Retire le rôle d'accès pendant 5 min
  // ============================================================
    ...
    try {
      ...
      if (prisonChannel) {
        await prisonChannel.send({
          ...
          embeds: [embed('#FF4444')
            ...
            .setDescription(`Tu as été emprisonné par <@${message.author.id}>.\nTu seras libéré dans **${dureeMin} minute${dureeMin > 1 ? 's' : ''}**.\n\nC'est le seul salon que tu peux voir pour le moment.`)
          ],
        });
      }
      ...
      setTimeout(async () => {
        try {
          ...
          if (!member) return console.warn(`[JAIL] ${target.user.tag} a quitté le serveur avant la fin de sa peine.`);
          ...
          if (prisonChannel) {
            await prisonChannel.send({
              embeds: [embed('#00FF66')
                .setTitle('🔓 Libéré !')
                .setDescription(`<@${target.id}> a purgé sa peine et retrouve l'accès au serveur. Bienvenue dans le monde libre.`)
              ],
            });
          }
          ...
          console.error('[JAIL] Erreur lors de la libération :', err.message);
        }
  ...
  // ============================================================
  //  🧠  IQTEST — Score d'IQ aléatoire (troll)
  // ============================================================
  ...
  '!iqtest': async (message) => {
    ...
    else                { verdict = '🪨 Roche. Tu es une roche. La roche a plus de neurones.';      color = '#FF0000'; }

    const e = embed(color)
      .setTitle(`🧠 Résultat IQ — ${target.displayName}`)
      .addFields(
  ...
  // ============================================================
  //  💊  COPE DU JOUR — Cope aléatoire avec réfutation
  // ============================================================

  '!cope-du-jour': async (message) => {
    if (copesData.cope.length === 0) {
      return message.reply('❌ Aucun cope dans la liste. Utilise `!add-cope` pour en ajouter.');
    }
  ...
  // ============================================================
  //  🌊  FLUIDE — Statut "système fluide" (troll admin)
  // ============================================================
  ...
  '!fluide': async (message) => {
    ...
    const motifs = [
      'comportement inexplicable détecté',
      'neurones dysfonctionnels confirmés',
      'cohérence logique introuvable',
      'ratio subi sans broncher',
      'a défendu un cope en public',
      'a demandé une source',
      'a mentionné le MK-677 volontairement',
      'analyse biométrique : QI fluide détecté',
    ...
    const e = embed('#9B59B6')
      .setTitle('🌊 Système Fluide Activé')
      .setDescription(
        `<@${target.id}> est officiellement passé sous **système fluide** pour les prochaines **24h**.\n\n` +
        `Conformément à la règle 3, les insultes, moqueries et ratios à son encontre sont désormais **autorisés et encouragés** jusqu'à rémission.`
      )
      .addFields(
